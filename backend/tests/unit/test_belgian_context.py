...
from app.models_stage1 import Child, User
...
class TestBelgianSchoolSystem:
    ...
    def test_belgian_class_levels(self, belgian_classes):
        ...
        total_levels = len(belgian_classes["maternelle"]) + len(
            belgian_classes["primaire"]
        )
...
class TestChildManagement:
    ...
    def test_create_child_with_belgian_class(
        self,
        client: TestClient,
        ...
        child_data = {"first_name": "Emma", "class_name": "P3"}

        response = client.post(
            "/api/children", json=child_data, headers=auth_headers_parent
        )

        assert response.status_code == 200
        data = response.json()
        assert data["first_name"] == "Emma"
        assert data["class_name"] == "P3"
        assert data["class_name"] in belgian_classes["primaire"]

    def test_invalid_class_level_rejected(
        self, client: TestClient, auth_headers_parent: dict, test_user_parent: User
    ):
        """Test invalid class level is rejected."""
        child_data = {"first_name": "Test", "class_name": "INVALID"}
    ...
    def test_get_children_by_parent(
        self, client: TestClient, auth_headers_parent: dict, test_child: Child
    ):
        """Test getting children for authenticated parent."""
        response = client.get("/api/children", headers=auth_headers_parent)
    ...
    def test_child_class_progression(
        self, db_session: Session, test_child: Child, belgian_classes
    ):
        """Test child can progress through Belgian class levels."""
        # Start in P3
        assert test_child.class_name == "P3"

        # Progress to P4
        test_child.class_name = "P4"
        db_session.commit()

        assert test_child.class_name == "P4"
        assert test_child.class_name in belgian_classes["primaire"]
...
class TestBelgianSELContext:
    ...
    def test_sel_units_system(self):
        ...
        assert (
            abs(minimum_balance) < maximum_balance
        )  # More generous giving than taking
