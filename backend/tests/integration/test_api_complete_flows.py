...
class TestCompleteUserJourney:
    ...
    def test_complete_parent_onboarding_flow(self, client: TestClient):
        ...
        child_data = {"first_name": "Emma", "last_name": "Journey", "class_level": "P4"}
        ...
        service_response = client.post(
            "/sel/services", json=service_data, headers=headers
        )
...
class TestSELTransactionFlow:
    ...
    def test_complete_sel_transaction_flow(
        ...
        service_response = client.post(
            "/sel/services", json=service_data, headers=provider_headers
        )
        ...
        transaction_response = client.post(
            "/sel/transactions", json=transaction_data, headers=requester_headers
        )
        ...
        approval_response = client.put(
            f"/sel/transactions/{transaction['id']}/approve", headers=provider_headers
        )
        assert approval_response.status_code == 200

        # Check updated balances after approval
        updated_provider_balance = client.get("/sel/balance", headers=provider_headers)
        updated_requester_balance = client.get(
            "/sel/balance", headers=requester_headers
        )
...
class TestMessageAndEventFlow:
    ...
    def test_message_service_owner_flow(self, client: TestClient):
        ...
        provider_data = {
            "email": "msgprovider@test.be",
            "first_name": "Msg",
            "last_name": "Provider",
            "password": "jules20220902",
        }
        requester_data = {
            "email": "msgrequester@test.be",
            "first_name": "Msg",
            "last_name": "Requester",
            "password": "jules20220902",
        }
        ...
        provider_login = client.post(
            "/login",
            data={"email": provider_data["email"], "password": "jules20220902"},
        )
        requester_login = client.post(
            "/login",
            data={"email": requester_data["email"], "password": "jules20220902"},
        )
        ...
        requester_headers = {
            "Authorization": f"Bearer {requester_login.json()['access_token']}"
        }
...
class TestAdminWorkflow:
    ...
    def test_admin_user_management(
        self, client: TestClient, test_user_admin: User, auth_headers_admin: dict
    ):
    ...
    def test_admin_service_moderation(
        self, client: TestClient, test_user_admin: User, auth_headers_admin: dict
    ):
        ...
        if services:
            ...
            deactivate_response = client.put(
                f"/admin/services/{service_id}/deactivate", headers=auth_headers_admin
            )
