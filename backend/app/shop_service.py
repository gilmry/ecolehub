...
from .models_stage3 import ShopProduct, ShopInterest, ShopOrder, ShopOrderItem
from .models_stage1 import User
...
class ShopCollaborativeService:
    ...
    def get_products(self, category: Optional[str] = None, active_only: bool = True) -> List[ShopProduct]:
    ...
    def get_product_with_interest_count(self, product_id: UUID) -> Dict[str, Any]:
        ...
        product = self.db.query(ShopProduct).filter(ShopProduct.id == product_id).first()
        if not product:
            raise HTTPException(status_code=404, detail="Produit non trouvé")
        
        # Count total interest
        total_interest = self.db.query(func.sum(ShopInterest.quantity)).filter(
            and_(
        ...
        interested_users = self.db.query(ShopInterest).filter(
            and_(
                ShopInterest.product_id == product_id,
                ShopInterest.status == 'interested'
            )
        ...
        progress_percentage = min((total_interest / product.min_quantity) * 100, 100) if product.min_quantity > 0 else 100
        can_order = total_interest >= product.min_quantity
        
        # Calculate Belgian price with tax
        tax_calculation = mollie_service.calculate_belgian_tax(float(product.base_price))
    ...
    def express_interest(
        ...
        existing_interest = self.db.query(ShopInterest).filter(
            and_(
                ShopInterest.product_id == product_id,
    ...
    def cancel_interest(self, user_id: UUID, product_id: UUID) -> Dict[str, Any]:
        ...
        interest = self.db.query(ShopInterest).filter(
            and_(
                ShopInterest.product_id == product_id,
                ShopInterest.user_id == user_id
            )
        ...
        return {
            "success": True,
            "message": "Intérêt annulé"
        }
    
    def create_group_order(self, product_id: UUID, admin_user_id: UUID) -> Dict[str, Any]:
        """
        ...
        interests = self.db.query(ShopInterest).filter(
            and_(
                ShopInterest.product_id == product_id,
                ShopInterest.status == 'interested'
            )
    ...
    def get_user_orders(self, user_id: UUID) -> List[Dict[str, Any]]:
        ...
        order_items = self.db.query(ShopOrderItem).filter(
            ShopOrderItem.user_id == user_id
        ).order_by(ShopOrderItem.created_at.desc()).all()
        ...
        for item in order_items:
            result.append({
                ...
                "estimated_delivery": item.order.estimated_delivery.isoformat() if item.order.estimated_delivery else None,
