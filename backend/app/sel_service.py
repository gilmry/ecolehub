...
from .models_stage1 import User, SELService, SELTransaction, SELBalance, SELCategory
from .schemas_stage1 import SELServiceCreate, SELTransactionCreate, TransactionStatus
...
class SELBusinessLogic:
    ...
    def get_or_create_balance(self, user_id: UUID) -> SELBalance:
        ...
        balance = self.db.query(SELBalance).filter(SELBalance.user_id == user_id).first()
        if not balance:
    ...
    def validate_transaction_balance(self, from_user_id: UUID, to_user_id: UUID, units: int) -> dict:
        """
    ...
    # Service Management
    def create_service(self, user_id: UUID, service_data: SELServiceCreate) -> SELService:
        ...
        if service_data.category == "proposition" and service_data.new_category_name:
            ...
            category = self.db.query(SELCategory).filter(SELCategory.name == service_data.category).first()
            if not category:
                raise HTTPException(status_code=400, detail=f"Catégorie '{service_data.category}' inexistante")
    ...
    def get_available_services(self, requesting_user_id: UUID, category: Optional[str] = None, limit: int = 50) -> List[SELService]:
        """Get available services (excluding own services)."""
        query = self.db.query(SELService).filter(
            and_(
                SELService.is_active == True,
    ...
    def create_transaction(self, from_user_id: UUID, transaction_data: SELTransactionCreate) -> SELTransaction:
        """
        ...
        if from_user_id == transaction_data.to_user_id:
            raise HTTPException(status_code=400, detail="Impossible de créer une transaction avec soi-même")
        
        from_user = self.db.query(User).filter(User.id == from_user_id).first()
        to_user = self.db.query(User).filter(User.id == transaction_data.to_user_id).first()
        ...
        if transaction_data.service_id:
            service = self.db.query(SELService).filter(
                and_(
    ...
    def approve_transaction(self, transaction_id: UUID, approving_user_id: UUID) -> SELTransaction:
        """
        Approve a transaction and update balances.
        Only the recipient (to_user) can approve.
        """
        transaction = self.db.query(SELTransaction).filter(SELTransaction.id == transaction_id).first()
        if not transaction:
            raise HTTPException(status_code=404, detail="Transaction non trouvée")
        
        # Only recipient can approve
        if transaction.to_user_id != approving_user_id:
            raise HTTPException(status_code=403, detail="Seul le destinataire peut approuver cette transaction")
        
        if transaction.status != "pending":
            raise HTTPException(status_code=400, detail=f"Transaction déjà {transaction.status}")
        ...
        if not validation["valid"]:
            raise HTTPException(status_code=400, detail=f"Transaction impossible: {validation['message']}")
    ...
    def cancel_transaction(self, transaction_id: UUID, cancelling_user_id: UUID) -> SELTransaction:
        """
        ...
        # Only sender or recipient can cancel
        if transaction.from_user_id != cancelling_user_id and transaction.to_user_id != cancelling_user_id:
            raise HTTPException(status_code=403, detail="Vous ne pouvez pas annuler cette transaction")
        
        if transaction.status != "pending":
            raise HTTPException(status_code=400, detail=f"Impossible d'annuler une transaction {transaction.status}")
    ...
    def get_user_dashboard(self, user_id: UUID) -> dict:
        ...
        active_services = self.db.query(func.count(SELService.id)).filter(
            and_(SELService.user_id == user_id, SELService.is_active == True)
        ).scalar()
        
        # Count pending transactions
        pending_transactions = self.db.query(func.count(SELTransaction.id)).filter(
            and_(
                or_(
        ...
        # Recent transactions
        recent_transactions = self.db.query(SELTransaction).filter(
            or_(
                SELTransaction.from_user_id == user_id,
                SELTransaction.to_user_id == user_id
            )
        ).order_by(desc(SELTransaction.created_at)).limit(10).all()
