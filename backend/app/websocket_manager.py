...
class WebSocketManager:
    ...
    async def broadcast_to_conversation(self, conversation_id: UUID, message_data: dict, sender_id: UUID = None):
        ...
        for user_id_str, subscriptions in self.user_subscriptions.items():
            if conversation_id_str in subscriptions and user_id_str != sender_id_str:
                if user_id_str in self.active_connections:
                    try:
                        await self.active_connections[user_id_str].send_text(message_json)
                    except:
    ...
    async def _handle_send_message(self, user_id: UUID, message_data: dict, db: Session):
    ...
    async def _handle_join_conversation(self, user_id: UUID, message_data: dict, db: Session):
    ...
    async def _handle_leave_conversation(self, user_id: UUID, message_data: dict, db: Session):
