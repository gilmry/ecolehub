...
import redis
from fastapi import Depends, FastAPI, Form, HTTPException, Query, WebSocket
...
from sqlalchemy import and_, create_engine, desc, func, text
...
def get_events(
    ...
    if not events:
        ...
        if first_event:
            print(
                f"📅 First event example: {first_event.title}, active: {first_event.is_active}, date: {first_event.start_date}"
            )
...
def register_for_event(
    ...
    if not event.registration_required:
        raise HTTPException(
            status_code=400, detail="Inscription non requise pour cet événement"
        )

    # Check deadline
    if event.registration_deadline and datetime.utcnow() > event.registration_deadline:
        raise HTTPException(
            status_code=400, detail="Date limite d'inscription dépassée"
        )
    ...
    participant = EventParticipant(
        event_id=event_id, user_id=current_user.id, notes=notes
    )
    db.add(participant)
    db.commit()
