...
from fastapi import FastAPI, Depends, HTTPException, Form, Header, Query, WebSocket, WebSocketDisconnect, File, UploadFile
...
def read_root():
    return {
        ...
        "école": "EcoleHub",
        "version": "3.0.0",
        "features": ["Authentication", "Profiles", "Children", "SEL System", "Messaging", "Events", "Shop", "Education"]
    }
...
def create_child(
    ...
    class_conv = db.query(Conversation).filter(
        and_(Conversation.type == 'class', Conversation.class_name == child.class_name.value)
    ).first()
    ...
    if class_conv:
        ...
        if not existing:
            participant = ConversationParticipant(conversation_id=class_conv.id, user_id=current_user.id)
            db.add(participant)
            db.commit()
...
def get_conversations(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    participations = db.query(ConversationParticipant).filter(ConversationParticipant.user_id == current_user.id).all()
    
    conversations = []
    for participation in participations:
        conversation = participation.conversation
        last_message = db.query(Message).filter(Message.conversation_id == conversation.id).order_by(desc(Message.created_at)).first()
...
def get_education_resources(
    ...
    query = db.query(EducationResource).filter(
        or_(
            EducationResource.is_public == True,
    ...
    for resource in resources:
        result.append({
            ...
            "creator_name": f"{resource.creator.first_name} {resource.creator.last_name}" if resource.creator else "École"
        })
