...
from .models_stage2 import Base, User
...
class ShopProduct(Base):
    ...
    interests = relationship(
        "ShopInterest", back_populates="product", cascade="all, delete-orphan"
    )
...
class ShopInterest(Base):
    ...
    id = Column(UUIDType(), primary_key=True, default=uuid.uuid4)
    product_id = Column(
        UUIDType(), ForeignKey("shop_products.id", ondelete="CASCADE"), nullable=False
    )
    user_id = Column(
...
class ShopOrder(Base):
    ...
    order_items = relationship(
        "ShopOrderItem", back_populates="order", cascade="all, delete-orphan"
    )
...
class ShopOrderItem(Base):
    ...
    id = Column(UUIDType(), primary_key=True, default=uuid.uuid4)
    order_id = Column(
        UUIDType(), ForeignKey("shop_orders.id", ondelete="CASCADE"), nullable=False
    )
...
class EducationResource(Base):
    ...
    )  # 'homework', 'calendar', 'forms', 'announcements'
...
class ResourceAccess(Base):
    ...
    resource_id = Column(
        UUIDType(),
        ForeignKey("education_resources.id", ondelete="CASCADE"),
        primary_key=True,
    )
    user_id = Column(
    ...
    __table_args__ = (
        CheckConstraint(
            "access_type IN ('read', 'write', 'admin')", name="valid_access_type"
        ),
    )
...
User.shop_products_created = relationship(
    "ShopProduct", foreign_keys="ShopProduct.created_by", back_populates="creator"
)
...
User.education_resources_created = relationship(
    "EducationResource",
