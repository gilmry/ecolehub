...
# Import models
from .models_stage1 import SELService, SELTransaction, User
from .models_stage2 import Event, EventParticipant, Message
from .models_stage3 import ShopInterest, ShopOrder, ShopProduct

# Prometheus metrics for EcoleHub
ecolehub_user_logins = Counter(
    "ecolehub_user_logins_total", "Total user logins", ["user_type"]
)
ecolehub_sel_transactions = Counter(
    "ecolehub_sel_transactions_total", "SEL transactions", ["status"]
)
ecolehub_shop_interests = Counter(
    "ecolehub_shop_interests_total", "Shop interest expressions", ["product_category"]
)
ecolehub_messages_sent = Counter(
    "ecolehub_messages_sent_total", "Messages sent", ["conversation_type"]
)
ecolehub_event_registrations = Counter(
    "ecolehub_event_registrations_total", "Event registrations", ["event_type"]
)
...
class EcoleHubAnalytics:
    ...
    def get_platform_overview(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_sel_transactions = self.db.query(
                func.count(SELTransaction.id)
            ).scalar()
            avg_sel_balance = (
                self.db.query(func.avg(SELTransaction.units)).scalar() or 0
            )
            ...
            return {
                "users": {
                    ...
                    "growth_rate": round(
                        (active_users_week / max(total_users, 1)) * 100, 1
                    ),
                },
                "sel": {
                    ...
                    "engagement_rate": round(
                        (total_sel_transactions / max(total_users, 1)), 2
                    ),
                },
                "shop": {
                    ...
                    "conversion_rate": round(
                        (total_orders / max(total_interests, 1)) * 100, 1
                    ),
                },
                "communication": {
    ...
    def get_user_analytics(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            user_services = (
                ...
                .filter(SELService.user_id == user_id)
                .scalar()
            )

            user_transactions_sent = (
                self.db.query(func.count(SELTransaction.id))
                .filter(SELTransaction.from_user_id == user_id)
                .scalar()
            )

            user_transactions_received = (
                self.db.query(func.count(SELTransaction.id))
                .filter(SELTransaction.to_user_id == user_id)
                .scalar()
            )

            # Shop activity
            user_interests = (
                self.db.query(func.count(ShopInterest.id))
                .filter(ShopInterest.user_id == user_id)
                .scalar()
            )
            ...
            user_event_registrations = (
                self.db.query(func.count(EventParticipant.user_id))
            ...
            return {
                ...
                "sel_activity": {
                    ...
                    "engagement_score": user_services
                    + user_transactions_sent
                    + user_transactions_received,
                },
                "shop_activity": {
    ...
    def get_shop_analytics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            popular_products = (
                ...
                .join(ShopInterest)
                .group_by(ShopProduct.id, ShopProduct.name)
                .order_by(func.count(ShopInterest.id).desc())
                .limit(5)
                .all()
            )
            ...
            category_stats = (
                ...
                .join(ShopInterest)
                .group_by(ShopProduct.category)
                .all()
            )

            # Order success metrics
            total_revenue = (
                self.db.query(func.sum(ShopOrder.total_price))
                .filter(ShopOrder.status.in_(["paid", "delivered"]))
                .scalar()
                or 0
            )
    ...
    def get_sel_analytics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            transaction_stats = (
                ...
                .group_by(SELTransaction.status)
                .all()
            )
            ...
            service_categories = (
                ...
                .group_by(SELService.category)
                .order_by(func.count(SELService.id).desc())
                .all()
            )
            ...
            monthly_transactions = (
                ...
                .group_by(func.date_trunc("month", SELTransaction.created_at))
                .order_by("month")
                .limit(12)
                .all()
            )
            ...
            return {
                ...
                "service_categories": [
                    {"category": cat.category, "count": cat.service_count}
    ...
    def track_user_action(
        ...
        try:
            ...
            if action == "login":
                ...
                ecolehub_sel_transactions.labels(
                    status=details.get("status", "unknown")
                ).inc()
            elif action == "shop_interest":
                ecolehub_shop_interests.labels(
                    product_category=details.get("category", "unknown")
                ).inc()
            elif action == "message_sent":
                ecolehub_messages_sent.labels(
                    conversation_type=details.get("type", "unknown")
                ).inc()
